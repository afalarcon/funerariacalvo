<?php 
	...
	$sql="SELECT sum(VrReci) as vr FROM `recibos` where fecha='$fech' and User = '$usue' and estado = 1";
			...
			$result->num_rows>0;
					...
					while ($row= $result->fetch_array()) {
						$valor=$row['vr'];
						 ...
						 $sql2="insert into cierre (UserEnv,UserRec,FecCier,VarCier ) values ('$usue','$usur','$fech',$valor)";
							$result1 = $mysqli->query($sql2);
				}


	$sql3="SELECT idCier  FROM `cierre` where FecCier='$fech' and UserEnv='$usue'";
			...
			$result3->num_rows>0;
					...
					while ($row3= $result3->fetch_array()) {
						$idcier=$row3['idCier'];
						 ...
						 $sql4="update recibos  set NroCierre = $idcier  where fecha='$fech' and User = '$usue'";
